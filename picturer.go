...
func (p picturer) GiveMePictureOf(query string) string {
	req, err := http.NewRequest("GET", "https://api.shutterstock.com/v2/images/search?sort=random&exclude_keywords=football,soccer,model,basketball&image_type=photo&query=" + query, nil)
	if err != nil {
		// handle error
	}
